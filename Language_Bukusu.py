...
def load_trained_model():
    ...
    try:
        model = load_model('/home/code/Desktop/MSC/Natural-Language-Processing/POS_luyha_project/version2_model/pos_model_explicit_initializers.h5', custom_objects=custom_objects)
        return model
...
# def load_trained_model():
#     custom_objects = {'Orthogonal': Orthogonal, 'CustomLSTM': CustomLSTM}
#     return load_model('/home/code/Desktop/MSC/Natural-Language-Processing/POS_luyha_project/version2_model/pos_model_explicit_initializers.h5', custom_objects=custom_objects)
...
#     # Preprocess the data for prediction
#     X = preprocess_data(data, word2idx)
...
#     # Convert indices to tags
#     data['PREDICTED_TAG'] = [' '.join([idx2tag[idx] for idx in pred if idx != 0]) for pred in predicted_tags]
    
#     st.write("Predictions:")
#     st.write(data.head())

#     # Basic analysis: Part of Speech count
#     pos_counts = data['PREDICTED_TAG'].str.split().apply(pd.Series).stack().value_counts()
#     st.write("Part of Speech Counts:")
...
#     # Preprocess user input
#     user_data = pd.DataFrame(user_input.split(), columns=["WORD"])
#     X_user = preprocess_data(user_data, word2idx)
...
#     # Convert indices to tags
#     user_data['PREDICTED_TAG'] = [' '.join([idx2tag[idx] for idx in pred if idx != 0]) for pred in predicted_tags_user]
...
# # Amosi we munaanie Bibilia
# # abaluhya ni Nyasaye nende 
